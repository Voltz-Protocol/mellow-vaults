...
contract<{}, DeployOptions, CustomContext>("Voltz E2E", function () {
    ...
    const doFixedTrade = async () => {
        // trade FT with some other account
        const { test } = await getNamedAccounts();
        const testSigner = await hre.ethers.getSigner(test);
        await mint(
            "USDC",
    ...
    const testFunction = async (swapsType1: number, swapsType2: number) => {
        expect(await this.usdc.balanceOf(this.user1.address)).to.be.eq(0);
        expect(await this.usdc.balanceOf(this.user2.address)).to.be.eq(0);
        ...
        if (swapsType1 == 1) {
            await doVariableTrade();
            let currentPosition = await this.voltzVault.currentPosition();
            await this.voltzVault.connect(this.voltzVaultOwner).rebalance({
                tickLower: currentPosition.tickLower,
                tickUpper: currentPosition.tickUpper,
            });
        } else if (swapsType1 == -1) {
            await doFixedTrade();
            let currentPosition = await this.voltzVault.currentPosition();
            await this.voltzVault.connect(this.voltzVaultOwner).rebalance({
                tickLower: currentPosition.tickLower,
        ...
        await this.erc20RootVault
            .connect(this.user2.signer)
            .withdraw(this.user2.address, this.user2.lpTokens, [0], [[], []]);
        const firstUserBalance = await this.usdc.balanceOf(this.user1.address);
        const secondUserBalance = await this.usdc.balanceOf(this.user2.address);
        const firstSwap =
            swapsType1 == 0 ? "NO" : swapsType1 == 1 ? "VT" : "FT";
        const secondSwap =
            swapsType2 == 0 ? "NO" : swapsType2 == 1 ? "VT" : "FT";
        const result =
            "Trading after the first deposit: " +
            firstSwap +
            ", Trading after the second deposit: " +
            secondSwap +
            ", First user balance: " +
            firstUserBalance.toString() +
            ", Second user balance: " +
            secondUserBalance.toString();
        return result;
    };
    describe("Integration test:", () => {
        it("", async () => {
            console.log(await testFunction(0, 0));
        });
        it("", async () => {
            console.log(await testFunction(0, 1));
        });
        it("", async () => {
            console.log(await testFunction(0, -1));
        });
        it("", async () => {
            console.log(await testFunction(1, 0));
        });
        it("", async () => {
            console.log(await testFunction(1, 1));
        });
        it("", async () => {
            console.log(await testFunction(1, -1));
        });
